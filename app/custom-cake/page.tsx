  ...
  return (
    <div className="builder-progress-wrap">
      ...
      <p className="builder-progress-label">
        ...
        <strong>{CUSTOM_STEP_LABELS[current]}</strong>
      </p>
    </div>
  );
}
...
export default function CustomCakePage() {
  ...
  async function handleFileSelect(file: File) {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("cake-reference-images").getPublicUrl(path);
      setImgUrl(publicUrl);
    } catch (e: any) {
                ...
                <div className="radio-grid">
                  {frostings.map((f) => (
                    <button
                      key={f.id}
                      onClick={() =>
                        setFrostingId(f.id === frostingId ? "" : f.id)
                      }
                    ...
                    >
                      ...
                      <div className="radio-content">
                        ...
                        {f.surcharge > 0 && (
                          <span className="radio-price">+${f.surcharge}</span>
                        )}
                      </div>
                    </button>
                  ))}
                  ...
                  {designs.map((d) => (
                    ...
                    >
                      ...
                      <div className="radio-content">
                        ...
                        {d.surcharge > 0 && (
                          <span className="radio-price">+${d.surcharge}</span>
                        )}
                      </div>
                    </button>
                  ))}
        ...
        return (
          <div className="builder-step-content">
            <div className="builder-step-hero">
              ...
              <p>
                We accommodate all dietary requirements with no compromise on
                taste.
              </p>
            </div>
            <div className="dietary-pick">
              <button
                ...
                className={`dietary-card${!eggless ? " active" : ""}`}
              >
                <span>🥚</span>
              ...
              >
                ...
                <span>
                  Vegetarian-friendly
                  {egglessOpt &&
                    egglessOpt.surcharge > 0 &&
                    ` (+$${egglessOpt.surcharge})`}
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="builder-step-content">
            ...
            <div className="cake-msg-wrap">
              <div className="cake-msg-header">
                <label htmlFor="cake-msg" className="option-label">
                  Your message
                </label>
            ...
            {!message.trim() && (
              <p className="builder-skip-hint">
                Leave blank to skip — our decorator will leave the cake
                unwritten.
              </p>
            )}
          </div>
      ...
      case 6:
        return (
          <div className="builder-step-content">
            ...
            <p className="builder-skip-hint">
              Optional — skip if you don&apos;t have a reference image.
            </p>
          </div>
        );
            ...
            {bundles.length > 0 && (
              <div className="builder-option-section">
                ...
                <p className="builder-section-hint">
                  Pick a bundle to save — or choose individual add-ons below.
                </p>
                    ...
                    >
                      <div className="bundle-head">
                        ...
                        {selectedBundleId === b.id && (
                          <Check size={16} className="bundle-check" />
                        )}
                      </div>
                      <p className="bundle-desc">{b.description}</p>
                      <div className="bundle-items">
                        {b.items.map((i) => (
                          <span key={i.id} className="bundle-tag">
                            {i.name}
                          </span>
                        ))}
                      </div>
                      <div className="bundle-price-row">
                        <span className="bundle-original">
                          ${b.total_original.toFixed(2)}
                        </span>
                        <span className="bundle-final">
                          ${b.total_discounted.toFixed(2)}
                        </span>
                        <span className="bundle-saving">
                          Save ${b.saving.toFixed(2)}
                        </span>
                      </div>
                    </button>
                  ))}
          ...
          <div className="builder-step-content">
            ...
            <div className="review-card">
              {[
                {
                  label: "Occasion",
                  value: occasion ? `${occasion.emoji} ${occasion.label}` : "—",
                  step: 0,
                },
                {
                  label: "Size",
                  value: weight ? `${weight.label} · ${weight.portions}` : "—",
                  step: 1,
                },
                {
                  label: "Flavor",
                  value: flavor ? `${flavor.emoji} ${flavor.name}` : "—",
                  step: 2,
                },
                {
                  label: "Frosting",
                  value:
                    frostings.find((f) => f.id === frostingId)?.name ??
                    "Standard",
                  step: 3,
                },
                {
                ...
                {
                  label: "Dietary",
                  value: eggless ? "🌿 Eggless" : "🥚 Standard",
                  step: 4,
                },
                {
                  label: "Message",
                  value: message.trim() || "No message",
                  step: 5,
                },
                {
                  label: "Reference",
                  value: imgUrl ? "✅ Image uploaded" : "No image",
                  step: 6,
                },
                {
                  label: "Add-ons",
            ...
            <div className="review-price-card">
              {isVerifying ? (
                <div className="price-verifying">
                  <Loader2 className="spinner" size={16} /> Calculating price…
                </div>
              ) : priceData ? (
                <>
                  {priceData.breakdown.map((b, i) => (
                    <div key={i} className="price-breakdown-row">
                      <span>{b.name}</span>
                      <span className={b.amount < 0 ? "price-discount" : ""}>
            ...
            <div className="review-qty-row">
              ...
              {priceData && (
                <span className="review-total-hint">
                  Total: ${(priceData.per_item_price * quantity).toFixed(2)}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="builder-step-content builder-final-step">
            {added ? (
              <div className="builder-success">
                ...
                <p>
                  Our bakers will review your order and confirm the details with
                  you.
                </p>
                <div className="builder-success-actions">
                  ...
                  <a href="/cakes" className="btn btn-outline">
                    Keep Shopping
                  </a>
                </div>
              </div>
              ...
              <>
                <div className="builder-step-hero">
                  ...
                  <h2>Add to Cart</h2>
                  <p>
                    Your custom cake will be reviewed by our team before baking
                    begins.
                  </p>
                </div>
                ...
                <div className="final-summary">
                  ...
                  <div className="final-summary-info">
                    <p className="eyebrow">
                      {occasion?.label ?? "Custom Cake"}
                    </p>
                    <h3>
                      {weight?.label} · {flavor?.name ?? "—"}
                    </h3>
                    {message && (
                      <p className="final-message">&ldquo;{message}&rdquo;</p>
                    )}
                    <div className="final-price">
                      {isVerifying ? (
                        <div className="price-verifying">
                          <Loader2 className="spinner" size={16} /> Calculating…
                        </div>
                      ) : priceData ? (
                        <span className="product-price">
                          ${(priceData.per_item_price * quantity).toFixed(2)}{" "}
                          CAD
                        </span>
                      ) : null}
                    </div>
          ...
          <div className="builder-card">
            ...
            {step < TOTAL_STEPS - 1 && !added && (
              <div className="builder-nav">
                {step > 0 && (
                  <button
                    className="btn btn-outline builder-back"
                    onClick={goBack}
                  >
            ...
            <div className="summary-lines">
              ...
              {message && (
                <div className="summary-line">
                  <span>Message</span>
                  <strong>
                    &ldquo;{message.slice(0, 24)}
                    {message.length > 24 ? "…" : ""}&rdquo;
                  </strong>
                </div>
              )}
              {selectedAddons.length > 0 && (
